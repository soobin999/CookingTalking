...
public class UserServiceImpl implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		if ( userEntity != null ) {
			authorities.add(new SimpleGrantedAuthority(userEntity.getAccess() == 1 ? Role.MEMBER.getValue() : Role.ADMIN.getValue()));
			// authorities.add(new SimpleGrantedAuthority 롤 부여코드
		} 
		return userEntity == null ? null : new org.springframework.security.core.userdetails.User(userEntity.getUserId(), userEntity.getUserPw(),
				authorities);
